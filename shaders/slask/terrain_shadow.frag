...
void main(void)
{
    ...
    const float k_a = 0.2;//0.25;
    const float k_d = 0.8;//0.75;
    const float k_s = 1.0;
    const float k_g = 0.1;
    ...
    const float waterHalfRange = 10.0f;
    ...
    if( height < 0 )
    {
        ...
        {
            outColor = clamp(exp(log(2)*height/waterHalfRange),0,1) * outColor; // Exponential model
    ...
    float depth = texture(shadowSampler, varShadowCoord.xy).r;
    if( varShadowCoord.z > depth + depthSensitivity )
    {
        outColor *= shadowFactor; // DEBUG : vec4(1,0,0,1);
        outColor.a = 1;
    }
    */
