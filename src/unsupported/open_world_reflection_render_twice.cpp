...
void draw(GLuint fbo, cd::GLMCamera & camera, GLbitfield flags = (GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT) )
{
...
void display()
{
    ...
    camera.bindShader(
            ...
            "mdlMatrix");
    glEnable(GL_BLEND);
    glUniform1f(glGetUniformLocation(water_shader, "alpha"), 0);
    water_mesh.draw();
    */
    glEnable(GL_DEPTH_TEST);
    ...
    camera.bindShader(
            ...
            "mdlMatrix");
    glEnable(GL_BLEND);
    glUniform1f(glGetUniformLocation(water_shader, "alpha"), 0.5);
    water_mesh.draw();
    // Draw original scene
    glClear(GL_DEPTH_BUFFER_BIT);
    glUseProgram(terrain_shader);
