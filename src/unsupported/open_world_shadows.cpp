...
void init(void)
{
    ...
    terrain_shader = loadShaders(
            "shaders/terrain_shadow.vert",
            "shaders/terrain_shadow.frag");
...
#define NUM_WAVELEN 16
    ...
    ground_mesh.translate(0,-map_height,0);
    ...
    */
    ...
    //glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, shadow_tex, 0);
    ...
    float bound = map_scale * map_size;
...
void updateTime()
{
    ...
    GLfloat th = 3*M_PI/2 + elapsed_time/30;
...
void updateCamera() {
    ...
    mouseX = 0;
    mouseY = 0;
    float bound = map_scale * map_size / 2;
    //camera.setOrtho(-bound,bound,-bound,bound,-bound,bound);
    //camera.lookTowards(glm::vec3(map_size*map_scale/2,0,map_size*map_scale/2), -sun_dir);
...
void display()
{
    ...
    /*
    std::vector<uint8_t> data(2*window_width*window_height);
    glReadBuffer(GL_DEPTH_ATTACHMENT);
    printError("glReadBuffer");
    glReadPixels(0,0,window_width,window_height,GL_BGRA,GL_FLOAT,&data[0]);
    cv::Mat tmp(window_height,window_width,CV_32FC1,&data[0]);
    double min, max;
