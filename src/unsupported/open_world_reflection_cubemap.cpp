...
void init(void)
{
    ...
    water_shader = loadShaders(
            "shaders/mirror_cubemap.vert",
            "shaders/mirror_cubemap.frag");
...
#define NUM_WAVELEN 16
    ...
    glGenTextures(1, &cube_map_render_tex);
    ...
    //cv::Mat img = cv::imread("data/textures/windX/wind100.png",-1);
    //std::cout << "Matrix type = " << cd::cvType2Str(img.type()) << std::endl;
    for(int face = 0; face < 6; ++face)
    { 
        //glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + face,0,GL_RGBA,img.cols,img.rows,0,GL_RGBA,GL_UNSIGNED_BYTE,img.data);
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + face,0,GL_RGBA,window_width, window_height,0,GL_RGBA,GL_UNSIGNED_BYTE,NULL);
    }
    printError("render glTexImage2D");

    /*
    glGenTextures(1, &cube_map_depth_tex);
    glBindTexture(GL_TEXTURE_CUBE_MAP, cube_map_depth_tex);
    glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE); 
    glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE); 
    ...
    for(int face = 0; face < 6; ++face)
    { 
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + face,0,GL_DEPTH_COMPONENT24,img.cols,img.rows,0,GL_DEPTH_COMPONENT,GL_FLOAT,NULL);
    }
    printError("render glTexImage2D");
    */
    ...
    */
    glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, cube_map_render_tex, 0);
    ...
    std::cout << "cube_map_render_tex = " << cube_map_render_tex << std::endl;
...
void display()
{
    ...
    */
    ...
    cam_pos.y = 0;
    // X+
    mirror_camera.lookTowards(cam_pos, glm::vec3(1,0,0)); 
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_POSITIVE_X, cube_map_render_tex, 0);
    draw(fbo,mirror_camera);
    // X-
    mirror_camera.lookTowards(cam_pos, glm::vec3(-1,0,0)); 
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_NEGATIVE_X, cube_map_render_tex, 0);
    draw(fbo,mirror_camera);
    // Z+
    mirror_camera.lookTowards(cam_pos, glm::vec3(0,0,1)); 
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_POSITIVE_Z, cube_map_render_tex, 0);
    draw(fbo,mirror_camera);
    // Z-
    mirror_camera.lookTowards(cam_pos, glm::vec3(0,0,-1)); 
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_NEGATIVE_Z, cube_map_render_tex, 0);
    draw(fbo,mirror_camera);
    // Y+
    mirror_camera.lookTowards(cam_pos, glm::vec3(0,1,0)); 
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_POSITIVE_Y, cube_map_render_tex, 0);
    draw(fbo,mirror_camera);
    // Y-
    mirror_camera.lookTowards(cam_pos, glm::vec3(0,-1,0)); 
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, cube_map_render_tex, 0);
    draw(fbo,mirror_camera);
