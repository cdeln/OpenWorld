...
void init(void)
{
    ...
    terrain_shader = loadShaders(
            "shaders/terrain_scatter.vert",
            "shaders/terrain_scatter.frag");
...
#define NUM_WAVELEN 16
    ...
    for(GLuint w = 0; w < NUM_WAVELEN; ++w)
    {
        ...
        for(GLuint w = 0; w < NUM_WAVELEN; ++w)
        {
            for(GLuint k = 0; k < 3; ++k)
            {
                std::cout << "tristimulus["<<k<<","<<w<<"] = " << tristimulus[k][w] << std::endl;
            }
        }
    }
    ...
    // Water 
    water_mesh = cd::createQuad(-map_size,-map_size,2*map_size,2*map_size); 
    water_mesh.toGPU();
...
void display(void)
{
    ...
    camera.bindShader(
            ...
            "projMatrix",
            "mdlMatrix");
    GLfloat th = M_PI/2 - elapsed_time/30;
