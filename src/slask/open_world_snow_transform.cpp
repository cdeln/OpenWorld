...
void init(void)
{
    ...
    */
    ...
    // Particles
    // Shader program 
    char * shader_src = readFile("shaders/snow_transform.vert");
    ...
    const GLchar* feedback_var[] = {"outPosition"};
    glTransformFeedbackVaryings(particle_transform_shader, 1, feedback_var, GL_INTERLEAVED_ATTRIBS);
    ...
    /*
       glGenBuffers(1,&particle_tbo);
       glBindBuffer(GL_ARRAY_BUFFER, particle_tbo);
       glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec3)*particles.size(), nullptr, GL_DYNAMIC_DRAW);
       printError("particle tbo");
       */

    VERBOSE(particle_vao[0]);
    VERBOSE(particle_vao[1]);
    VERBOSE(particle_vbo[0]);
    VERBOSE(particle_vbo[1]);
